...
namespace utils
{
	...
	public class ResMgr_Lua 
	{
		...
		{
			var obj = ResMgr.Load(lua.Api.lua_tostring(L, 1));
		...
		{
			var objs = ResMgr.LoadAll(lua.Api.lua_tostring(L, 1));
		...
		{
			var sprite = ResMgr.Load<Sprite>(lua.Api.lua_tostring(L, 1));
		...
		{
			var sprites = ResMgr.LoadAll<Sprite>(lua.Api.lua_tostring(L, 1));
		...
		{
			var tex = ResMgr.Load<Texture2D>(lua.Api.lua_tostring(L, 1));
